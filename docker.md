...
docker run --gpus all --ipc=host --shm-size=16g \
  ...
  -v /ssd2/zhizhou/workspace/rotation-project:/root \
  --name slime_zhizhou \
...
```bash
# 这个会把 llamafactory 的 image 放在后台
docker run -itd --gpus=all --ipc=host \
  -v /ssd2/zhizhou/workspace/LLaMA-Factory:/app \
  --name zhizhou_llamafactory \
  hiyouga/llamafactory:latest \
  /bin/bash
  
 # 连接到正在运行的容器
 docker exec -it zhizhou_llamafactory bash
```
...
```bash
docker run -itd --gpus=all --ipc=host \
  -v /home/zs7752/.local/bin/claude:/usr/local/bin/claude \
  -v /home/zs7752/.claude:/root/.claude \
  -v /ssd2/zhizhou/workspace/mcpmark:/app \
  -e PATH="/usr/local/bin:$PATH" \
  -e HOME=/root \
  -e USER=root \
  --name zhizhou_mcpmark \
  evalsysorg/mcpmark:latest \
  /bin/bash

docker exec -it zhizhou_mcpmark bash

docker rm -f zhizhou_mcpmark
```



现在我的做法是 linux 机器上先开一个 tmux，然后 tmux 里边连 docker。这样就可以一直挂着了

如果要停掉一个 container 的话，得先 kill，然后 rm，才能重新用它的名字
```bash
# 一条指令版
